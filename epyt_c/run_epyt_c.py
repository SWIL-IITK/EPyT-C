...
index_reservoirs = fn.network(d)[8]; index_tanks = fn.network(d)[9]; index_pumps = fn.network(d)[10]; index_valves = fn.network(d)[11]
unit = fn.network(d)[12]; link_connectivity_array = fn.network(d)[13]; connectivity_mat = fn.network(d)[14]
length_links = fn.network(d)[15]; diameter_links = fn.network(d)[16]; start_node_mat = fn.network(d)[17]; end_node_mat = fn.network(d)[18]
num_omitted_nodes = fn.network(d)[19]; index_omitted_nodes = fn.network(d)[20]; num_omitted_links = fn.network(d)[21]; index_omitted_links = fn.network(d)[22]
wq_parameter_num = module.species()[0]; bulk_wq_parameter_num = module.species()[1]; wall_wq_parameter_num = module.species()[2]; num_variables = module.species()[3]
...
fn.simulation_info(wq_max_iteration, wq_sim_days, wq_sim_time_step_s, total_wq_steps)
...
while wq_iteration_count <= wq_max_iteration:
    ...
    link_conc_data_points_max = fn.maximum_segments(Tolerable_u, wq_sim_time_step_s, length_links, H.Velocity)
    ...
    while wq_time <= wq_sim_days * 24 * 3600:
        ...
        if wq_time == 0:
            h_step_expected = 0
        sync_out = fn.sync_time(d, H, wq_time, total_h_steps_expected, base_time_cycle_s, \
        ...
        for p in range(num_links):
            if index_omitted_links.count(p + 1) > 0:
                ...
                if index_pumps.count(p + 1) > 0 or index_valves.count(p + 1) > 0:
                    length_links[p] = fn.minimum_link_length(total_h_steps_expected, wq_sim_time_step_s, Tolerable_u, H.Velocity)
                    diameter_links[p] = fn.minimum_link_diameter(num_links, num_pumps, num_valves, diameter_links)
                ...
                if length_links[p] != 0:
                    ...
                    for i in range(first_grid, last_grid):
                        ...
                        if index_pumps.count(p + 1) == 0 and index_valves.count(p + 1) == 0:
                            delta = module.pipe_reaction(wq_sim_time_step_s, p, i, link_flow_velocity, diameter_links[p], length_links[p], link_segments_width, variable_values_mat[wq_iteration_count - 1], link_conc_array_a)
        ...
        for n in range(num_nodes):
            if index_omitted_nodes.count(n + 1) > 0:
                ...
                if index_reservoirs.count(n + 1) > 0:
                    ...
                    links_connecting_from_node = fn.outgoing_links(n, start_node_mat)
                    ...
                    # Reactions
                    delta = module.tank_reaction(wq_sim_time_step_s, wq_step, n, tank_flow_volume_prev, tank_flow_volume, variable_values_mat[wq_iteration_count - 1], node_conc_initial_mat, node_conc_array_a)
                    for sp in range(bulk_wq_parameter_num):
