...
class TestEpytcModule2():
    """
    Class to test the functions used in bacterial_regrowth.py
    """
    ...
    def test_species(self):
        ...
        assert module.species()[3] == 19 # count of model variables
    ...
    def test_variables(self, maximum_iterations_count, variables_count):
        ...
        variables_count = 19 # count of selected MSRT model variables
        ...
        assert np.logical_and(-12.6 <= module.variables(maximum_iterations_count, variables_count)[:, 4], \
                       module.variables(maximum_iterations_count, variables_count)[:, 4] <= -7.0).all()
        assert np.logical_and(-3 <= module.variables(maximum_iterations_count, variables_count)[:, 5], \
                       module.variables(maximum_iterations_count, variables_count)[:, 5] <= 0.19).all()  
        assert (module.variables(maximum_iterations_count, variables_count)[:, 6] <= 5).all()
        assert (module.variables(maximum_iterations_count, variables_count)[:, 7] <= 6e-3).all()
        assert (module.variables(maximum_iterations_count, variables_count)[:, 8] <= 0.15).all()
        assert np.logical_and(-5 <= module.variables(maximum_iterations_count, variables_count)[:, 10], \
                       module.variables(maximum_iterations_count, variables_count)[:, 10] <= 0.5).all()
        assert (module.variables(maximum_iterations_count, variables_count)[:, 11] <= 2.2e-6).all()
        assert (module.variables(maximum_iterations_count, variables_count)[:, 12] <= 46).all()
        assert (module.variables(maximum_iterations_count, variables_count)[:, 13] == 12.5e-10).all()
        assert np.logical_and(7.8e-10 <= module.variables(maximum_iterations_count, variables_count)[:, 14], \
                       module.variables(maximum_iterations_count, variables_count)[:, 14] <= 11.5e-10).all()
        assert np.logical_and(6.7e-10 <= module.variables(maximum_iterations_count, variables_count)[:, 15], \
                       module.variables(maximum_iterations_count, variables_count)[:, 15] <= 12.1e-10).all()
        assert np.logical_and(8e-10 <= module.variables(maximum_iterations_count, variables_count)[:, 16], \
                       module.variables(maximum_iterations_count, variables_count)[:, 16] <= 34e-10).all()
        assert (module.variables(maximum_iterations_count, variables_count)[:, 17] == 1e-13).all()
        assert (module.variables(maximum_iterations_count, variables_count)[:, 18] == 9.31e-7).all()
    ...
    def test_pipe_reaction(self, water_quality_step, pipe_number, grid_number, pipe_velocity, pipe_diameter,\
        ...
        variables_matrix = module.variables(200, 19)[2] # Variable values corresponding to 3rd iteration
    ...
    def test_reservoir_quality(self, d, maximum_iterations_count, source_quality_matrix, pattern_input_command1, \
        ...
        """
        maximum_iterations_count = 200 # times for which the water qality simulation will be iteratively performed
        source_quality_matrix = [[1, 1, 0.5, 10, 0],[1, 1, 0.5, 10, 0]] # quality at the two sources of Net3 (boundary condition)
        pattern_input_command1 = 'none', 'rand' 
        percentage_variation_random_pattern1 = 0, 0.2 # 0 and 20% variations
        
        val = module.reservoir_quality(d, maximum_iterations_count, source_quality_matrix, pattern_input_command1[0], \
                                 percentage_variation_random_pattern1[0])
        ...
        assert np.logical_and(0.4 <= val[0][:, 2], val[0][:, 2] <= 0.6).all()
        assert np.logical_and(8 <= val[0][:, 3], val[0][:, 3] <= 12).all()
        assert (val[0][:, 4] == 0).all()
        assert np.logical_and(0.8 <= val[1][:, 0], val[1][:, 0] <= 1.2).all()    
        assert np.logical_and(0.8 <= val[1][:, 1], val[1][:, 1] <= 1.2).all()
        assert np.logical_and(0.4 <= val[1][:, 2], val[1][:, 2] <= 0.6).all()
        assert np.logical_and(8 <= val[1][:, 3], val[1][:, 3] <= 12).all()
    ...
    def test_reservoir_pattern(self, d, base_time_days, pattern_input_command2, percentage_variation_random_pattern2, \
        ...
        val = module.reservoir_pattern(d, base_time_days, pattern_input_command2[2], percentage_variation_random_pattern2[1], \
                                   reservoir_injection_start_step, reservoir_injection_end_step, reservoir_injection_input)
    ...
    def test_injection_quality(self, maximum_iterations_count, injection_node_index_matrix, injection_node_quality_matrix, \
        ...
        val = module.injection_quality(maximum_iterations_count, injection_node_index_matrix, injection_node_quality_matrix, \
                                   pattern_input_command1[0], percentage_variation_random_pattern1[0])
    ...
    def test_injection_pattern(self, d, base_time_days, injection_node_index_matrix, pattern_input_command2, percentage_variation_random_pattern2,\
        ...
        val = module.injection_pattern(d, base_time_days, injection_node_index_matrix, pattern_input_command2[2], percentage_variation_random_pattern2[1],\
                                   injection_node_injection_start_step, injection_node_injection_end_step, injection_node_injection_input)
...
test.test_variables(200, 19)
test.test_pipe_reaction(300, 1, 2, 0.5, 200, 100, 16.667, module.variables(200, 19)[2], np.ones((5, 5, 119)))
test.test_tank_reaction(300, 10, 96, 19358, 20172, module.variables(200, 19)[2], np.zeros((97, 5)), np.ones((5, 2881, 97)))
...
test.test_injection_quality(200, [21], [[2, 0, 0, 0, 0]], ('none', 'rand'), (0, 0.2))
